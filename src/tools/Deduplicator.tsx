...
interface Result { headers: string[]; rows: CellValue[][]; dupes: number; }
...
export function Deduplicator() {
  ...
  async function handleFile(f: File) {
    try {
      ...
      setFile(data); setKeys([]); setResult(null);
  ...
  function handleRun() {
    ...
    if (keep === 'flag') {
      ...
      const headers = [...file.headers, '_duplicate'];
    ...
    const outRows = keep === 'first'
      ? ordered.map(o => o.row)
      : file.rows.filter(row => seen.get(makeKey(row)) === row);

    const dupes = file.rowCount - outRows.length;
    setResult({ headers: [...file.headers], rows: outRows, dupes });
    addToast(`Found ${dupes} duplicate${dupes !== 1 ? 's' : ''}`, 'success');
  }

  function handleDownload() {
    if (!result || !file) return;
    const suffix = keep === 'flag' ? '_flagged' : '_deduped';
    const ext = format === 'xlsx' ? '.xlsx' : '.csv';
    download([result.headers as CellValue[], ...result.rows], `${stemName(file.file)}${suffix}${ext}`, format);
            ...
            <div className="flex gap-3 flex-wrap">
              {(['first', 'last', 'flag'] as KeepMode[]).map(m => (
                <label key={m} className="flex items-center gap-1.5 cursor-pointer">
                  <input type="radio" value={m} checked={keep === m} onChange={() => { setKeep(m); setResult(null); }} className="w-3.5 h-3.5 accent-violet-600" />
          ...
          <div className="space-y-4">
            <div className="flex gap-4 flex-wrap">
              <div className="bg-violet-50 dark:bg-violet-950/40 rounded-xl px-5 py-3 text-center">
                <p className="text-2xl font-bold text-violet-600 dark:text-violet-400">{result.dupes}</p>
              ...
              <div className="bg-slate-50 dark:bg-slate-800 rounded-xl px-5 py-3 text-center">
                <p className="text-2xl font-bold text-slate-700 dark:text-slate-300">{result.rows.length}</p>
            ...
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <button onClick={handleDownload} className="px-5 py-2 bg-violet-600 text-white text-sm font-semibold rounded-lg hover:bg-violet-700 transition-colors">
                Download
              </button>
            </div>
          </div>
